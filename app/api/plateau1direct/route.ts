...
export async function GET() {
  ...
  const fluxQuery = `from(bucket: "PLATEAU1") |> range(start: ${f1})   |> filter(fn: (r) => r._field == "Ptot") |> drop(columns: ["_start", "table","_stop", "_measurement", "device", "SSID","result" ])   |> truncateTimeColumn(unit: 10s) |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")`;

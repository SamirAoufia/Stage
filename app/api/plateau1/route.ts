...
export async function GET() {
  ...
  const date = `from(bucket: "PLATEAU1") |> range(start: ${f1} , stop: ${f2})   |> filter(fn: (r) => r._field == "Ptot") |> drop(columns: [ "table", "_measurement", "device", "SSID","result" ])  |> truncateTimeColumn(unit: 10s)  |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")`;

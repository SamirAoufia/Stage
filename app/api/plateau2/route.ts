...
export  async function GET() {
  ...
  const fluxQuery = 'from(bucket: "PLATEAU2") |> range(start: -5m)   |> filter(fn: (r) => r._field == "Ptot") |> drop(columns: ["_start", "table","_stop", "_measurement", "device", "SSID","result" ])  |> truncateTimeColumn(unit: 10s) |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")';

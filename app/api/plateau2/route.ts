...
export async function GET() {
  ...
  const date = `from(bucket: "PLATEAU2") |> range(start: ${f1} , stop: ${f2})   |> filter(fn: (r) => r._field == "Ptot") |> drop(columns: ["_start", "table","_stop", "_measurement", "device", "SSID","result" ])   |> pivot(rowKey:["_time"], columnKey: ["_field"], valueColumn: "_value")`;

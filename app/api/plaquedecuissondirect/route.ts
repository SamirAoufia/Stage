...
export async function GET() {
  ...
  const fluxQuery = `from(bucket: "PLAQUE_DE_CUISSON") |> range(start: ${f1})   |> drop(columns: ["_start", "_stop", "_field", "table", "tagname1", "result", "table"])  |> truncateTimeColumn(unit: 10s) |> pivot(rowKey:["_time"], columnKey: ["_measurement"], valueColumn: "_value")`
